  ...
  protected async run(input: any, config: RunnableConfig): Promise<T> {
    ...
    if (this.isSendInput(input)) {
      ...
      if (toolCallsToProcess.length === 0) {
        const additionalKwargsToolCalls = aiMessage.additional_kwargs
          ?.tool_calls as
          | Array<{
              id?: string;
              type?: string;
            ...
            .map((tc, index) => {
              ...
              const name =
                tc.function?.name && tc.function.name.length > 0
                  ? tc.function.name
                  : savedInfo?.name || '';
              let id = tc.id && tc.id.length > 0 ? tc.id : savedInfo?.id || '';
      ...
      if (this.loadRuntimeTools) {
        const { tools, toolMap } = this.loadRuntimeTools(toolCallsToProcess);
        this.toolMap =
...
export function toolsCondition<T extends string>(
  ...
  const additionalKwargsToolCalls = message.additional_kwargs?.tool_calls as
    | Array<{
        id?: string;
        type?: string;
        function?: { name?: string; arguments?: string };
  ...
  if (additionalToolCalls > 0 && standardToolCalls === 0) {
    ...
    hasUnparsedToolCalls =
      additionalKwargsToolCalls?.some((tc) => {
        const funcName = tc?.function?.name;
        const funcArgs = tc?.function?.arguments;
        // Has valid function call data (name or arguments)
        return (
          (funcName && funcName.length > 0) || (funcArgs && funcArgs.length > 0)
        );
      }) ?? false;
  }

  const hasToolCalls =
    ('tool_calls' in message && (message.tool_calls?.length ?? 0) > 0) ||
    hasUnparsedToolCalls;
  const toolCallsInvoked = areToolCallsInvoked(message, invokedToolIds);

  if (hasToolCalls && !toolCallsInvoked) {
    return toolNode;
  } else {
    return END;
  }
