...
const { generateChartFromToolResult } = require("~/server/services/Charts/chartGenerationGuard");
 ...
 * @param {Array} [params.contentParts] Aggregated response parts, used for deterministic chart post-conditions.
 * @returns {ToolEndCallback} The tool end callback.
 */
function createToolEndCallback({ req, res, artifactPromises, contentParts }) {
  const chartCheckedToolCalls = new Set();
  /**
  ...
  return async (data, metadata) => {
    ...
    // Every agent/MCP tool completion passes through this gate. The model does
    // not need to remember to call a chart tool; chartability is decided from
    // the returned structured rows and the approved ECharts templates are
    // materialized deterministically.
    const toolName = output.name || output.tool_call?.name || metadata?.tool_name;
    const toolCallId = output.tool_call_id || metadata?.tool_call_id;
    const isChartTool = toolName === "chart_generator" || toolName === "echarts_generator" || toolName === "echarts_generator_app";
    const isTransfer = typeof toolName === "string" && toolName.includes("transfer_to_");
    if (!isChartTool && !isTransfer && toolCallId && !chartCheckedToolCalls.has(toolCallId)) {
      chartCheckedToolCalls.add(toolCallId);
      const chartOutput = generateChartFromToolResult(output.content ?? output.artifact ?? output, {
        agentName: metadata?.name,
        toolName,
        chartId: `auto_${toolCallId}`,
      });
      if (chartOutput && Array.isArray(contentParts)) {
        const syntheticToolCall = {
          id: `auto_chart_${toolCallId}`,
          name: "echarts_generator",
          args: { charts: chartOutput.charts },
          type: "tool_call",
          progress: 1,
          output: JSON.stringify(chartOutput),
          autoGenerated: true,
        };
        contentParts.push({ type: ContentTypes.TOOL_CALL, tool_call: syntheticToolCall });
        if (res?.headersSent) {
          res.write(`event: on_run_step_completed\\ndata: ${JSON.stringify({ data: { result: { tool_call: syntheticToolCall } } })}\\n\\n`);
        }
      }
    }
