...
/**
 * 多策略 JSON 解析：直接解析 → 修复尾部逗号 → 修复缺少引号的 key。
 * 覆盖 LLM 最常见的三类 JSON 错误。
 */
function repairAndParseJson(jsonStr) {
  ...
  // 匹配: { 或 , 之后、: 之前，缺少引号的 key
  // 覆盖: {key:、{key":、,key:、,key": 四种情况
  // 不会误伤已有的正确格式 {"key":
  try {
    let fixed = jsonStr.replace(/,\s*([}\]])/g, "$1");
    // 先修复缺少开头引号的情况: key": → 补开头引号（避免下面贪心匹配吃掉引号）
    fixed = fixed.replace(
...
function coerceToChartsArray(value) {
  ...
  for (let i = 0; i < 3 && typeof v === "string"; i++) {
    ...
    // 提取 JSON 核心（跳过可能的前导 \\n 等）
    const core = extractJsonCore(trimmed);
    const parsed = repairAndParseJson(core);
    if (parsed !== undefined) {
      v = parsed;
    } else {
      // 所有修复策略失败，保持原样交给 zod 报错
      break;
    }
  }
...
class EChartsGeneratorAPP extends Tool {
  ...
  async _call(input) {
    ...
    try {
      ...
      let { charts } = input;
      charts = coerceToChartsArray(charts);
